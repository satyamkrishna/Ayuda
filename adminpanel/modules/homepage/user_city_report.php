<?php
	...
	$result = $db->ud_getQuery('SELECT * FROM ud_general_city WHERE city_id IN (SELECT DISTINCT(userCityID) FROM ud_user WHERE userCityID IS NOT NULL)');
	$cities = $db->ud_mysql_fetch_assoc_all($result);
	$city_all = array('city_id' => 'all','city_name' =>'All' );
	...
	{
		$result = $db->ud_whereQuery('ud_user',NULL,array('userCityID'=>$city_id));
		$user = $db->ud_mysql_fetch_assoc_all($result);
	}
		...
		<div class="row">
			<div class="large-4 columns">
				...
				for($i=0;$i<sizeof($cities);$i++)
				{
					if(isset($_GET['city']) && !empty($_GET['city']))
					{
						if($cities[$i]['city_id'] == $_GET['city'])
						{
							echo '<option selected="selected" value="'.$cities[$i]['city_id'].'">'.$cities[$i]['city_name'].'</option>';
						}
						else 
						{
							echo '<option value="'.$cities[$i]['city_id'].'">'.$cities[$i]['city_name'].'</option>';	
						}
					}
					else 
					{
...
<div id="url" style="display:none;"><?php echo $_SERVER['PHP_SELF'].'?city='; ?></div>
...
<script src="../../resources/js/backend/manage_city_report.js"></script>
</body>
